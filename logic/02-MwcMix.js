...
var touchToStart = document.getElementById('touchToStart');
...
window.onload = function() {
    /*$.getJSON( "sampleJson.json", function( data ) { //Put JSON link here!
...
function animateDial(targetValue) {
    TweenMax.set([c, renagadeText, realistText, tts1], {scale:.8, opacity:0, transformOrigin:'50% 50%'});
    TweenMax.to(c, 2, {scale:1, opacity:1, ease:Power1.easeOut, delay:1, transformOrigin:'50% 50%'});
    TweenMax.to([renagadeText, realistText, tts1], 1, {scale:1, opacity:1, ease:Power1.easeOut, delay:1, transformOrigin:'50% 50%'});
    TweenMax.to(this, 7.5, {currentPercentage:targetValue, onUpdate:intervalCircleDraw, ease:Power1.easeInOut, delay:1, onComplete:nextPage});
}

function nextPage() {
    TweenMax.to(c, .7, {scale:.8, opacity:0, ease:Power1.easeIn, delay:1, transformOrigin:'50% 50%', onComplete:function() {
        window.location.href = '03-HowDoYouCompare.php';
    }});
    TweenMax.to([renagadeText, realistText, tts1], .5, {scale:.8, opacity:0, ease:Power1.easeIn, delay:1, transformOrigin:'50% 50%'});
}
